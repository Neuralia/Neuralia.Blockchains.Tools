...
namespace Neuralia.Blockchains.Tools.Data {
	...
	public class SafeArrayHandle : SafeHandle<ByteArray, SafeArrayHandle>, IPoolEntry, IComparable<SafeArrayHandle>, IComparable<byte[]>, IComparable<ByteArray> , IDisposable{

		private SafeArrayHandle() {
		}
		...
		public static new SafeArrayHandle Create() {
			var entry = HandlePool.GetObject();
			//entry.intstack = Environment.StackTrace;
			return entry;
		}
		...
		public static new SafeArrayHandle CreateSafeRandom(int size) {
		...
		public static new SafeArrayHandle CreateIncremental(int size) {
			var entry = Create(size);
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static SafeArrayHandle CreateZeroArray() {
			return (SafeArrayHandle) ByteArray.WrapAndOwn(Array.Empty<byte>());
		}
		...
		public override void Dispose() {
			...
			HandlePool.PutObject(this, (i) => i.Entry = null);
		}
