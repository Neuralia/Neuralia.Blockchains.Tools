...
namespace Neuralia.Blockchains.Tools.Threading{
    ...
    public static class ParallelAsync{
        public static async Task ForEach<T>(IEnumerable<T> collection, Func<(T entry, int index), Task> lambda,
            int? maxDegreeOfParallelism = null){
            if (collection == null){
                return;
            }

            var entries = collection.ToArray();

            if (!maxDegreeOfParallelism.HasValue){
                maxDegreeOfParallelism = Math.Max(Environment.ProcessorCount / 2, 1);
            }

            var transformBlock = new ActionBlock<(T entry, int index)>(
                lambda, new ExecutionDataflowBlockOptions{
